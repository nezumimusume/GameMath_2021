...
int WINAPI wWinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPWSTR lpCmdLine, int nCmdShow)
{
	...
	starPosition[0].Set(-400.0f, -100.0f, 0.0f);
	starPosition[1].Set(-200.0f, 0.0f, 0.0f);
	starPosition[2].Set(0.0f, 100.0f, 0.0f);
	starPosition[3].Set(200.0f, 0.0f, 0.0f);
	starPosition[4].Set(400.0f, -100.0f, 0.0f);
	...
	while (DispatchWindowMessage())
	{
		...
		for (int i = 0; i < 5; i++) {
			starPosition[i].z += 5.0f * moveDirection;
			starRender[i].SetPosition(starPosition[i]);
		}
